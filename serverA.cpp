...
/** This function reads a given file "a.txt".
...
int sendName(){
    ...
    cout << "ServerA finished sending a list of usernames to Main Server.\n";
 ...
 *  If the first character is '$', it means serverA needs to update its time intervals.
 *  If the first character is not '$', it means serverA needs to find the intersection for the query names.
...
int openServer(){
    ...
    while(1){
        ...
        if(buf[0] == '$') updateData((string)buf);
            ...
            cout << "Server A received the usernames from Main Server using UDP over port 21172.\n";
            ...
            cout << "Server A finished sending the response to Main Server.\n";
        }
    }
    return 0;
}
...
/** Initial a UDP socket that ServerA can connmunicate with Main Server
...
int initialSockA(){
    ...
    cout << "ServerA is up and running using UDP on port 21172.\n";
